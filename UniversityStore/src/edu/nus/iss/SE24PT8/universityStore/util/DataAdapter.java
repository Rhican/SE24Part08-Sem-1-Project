...
/**
 *
 * @author hendry Few method added by Mugunthan
 */
public class DataAdapter {
	...
	// SimpleDateFormat(DateTimeFormatter.ISO_LOCAL_DATE);
	public static final DateFormat DF = new SimpleDateFormat("yyyy-M-dd");

	// ==============Start of Category Tasks-=======================
	/*
	 * File Structure for Category 0-Category ID 1-Category Name;
	 * 
	 */
	...
	/*
	 * File Structure for Vendor 0-Vendor Name 1-Vendor Description;
	 */
	...
	/*
	 * File Structure for Discount 0-Discount Code 1-Discount Description
	 * 2-Start Date 3-Period 4-Percentage 5-Applicable for
	 * 
	 */
	...
	public static String convertDiscountToString(Discount object) {
		...
		return object.getDiscountCode() + comma + object.getDiscountDes() + comma + token_StartDate + comma
				+ token_Period + comma + object.getDiscountPercent() + comma + object.getApplicableFor();
	}
	...
	/*
	 * File Structure for Member 0-Member ID 1-Member Name 2-Loyalty Point
	 * 
	 */
	...
	/*
	 * File Structure for Product 0-Product ID 1-Product Name 2-Product
	 * Description 3-QuantityAvailable 4-Price 5-Barcode Number 6-Reorder
	 * Quantity 7-Order Quantity
	 * 
	 */
	...
	private static Product convertTokenstoProduct(ArrayList<String> tokens) {
		...
		try {
			...
			Product product = new Product(token0_ProductID, token1_ProductName, token2_ProductDesctipion,
					quantityAvailable, price, token5_Barcode, quantityReoder, quantityOrder);
			return product;
	...
	public static String convertProductToString(Product object) {
		return object.getProductId() + comma + object.getProductName() + comma + object.getBriefDesp() + comma
				+ object.getQty() + comma + object.getPrice() + comma + object.getBarcode() + comma
	...
	/*
	 * File Structure for StoreKeeper 0-StoreKeeperID 1-password
	 * 
	 * 
	 */
	...
	/*
	 * File Structure for Transaction 0-StoreKeeperID 1-password
	 * 
	 * 
	 */
	private static Transaction convertTokenstoTransaction(ArrayList<ArrayList<String>> saleItems) throws TransactionException {
		...
		for (ArrayList<String> saleItem : saleItems) {
			if (saleItem.size() != 5) {
				throw new TransactionException("Error: Transaction tokens size need to be 4 exact.");
			}
			...
			try {
				...
				throw new TransactionException("Transaction tokens Exception: " + e.getMessage(), e);
			}
		}
		
		try {
			transaction.close(true, true);
		} catch(TransactionException ex) {
			throw new TransactionException("Transaction Exception: " + ex.getMessage(), ex);
		}
		...
		for (int start = 0; start < dataLines.size(); start++) {
			...
			try {
				...
				if (!idString.equals(String.valueOf(transaction.getId()))) {
					System.out.println(
							"Warning: Transaction ID Incorrect, expected: " + transaction.getId() + " but " + idString);
				}
			} catch (TransactionException e) {
				System.out.println( "Warning: Transaction Loading fail \n" + e.getMessage());
			}			
		} // end of For loop
		return result;
	}
	...
	public static void testRetrieve() {
		...
		for (Transaction trans : transactions) {
			...
			if (trans.getMember() != null)
				System.out.println("Transaction" + trans.getId() + comma + trans.getMemberID() + comma
						+ DF.format(trans.getDate()));
			else
