...
/**
* Checkout Main panel 
* 	It will host CheckoutProductPanel and CheckoutMemberPanel 
* 
* @author Zehua
*/
public class CheckoutPanel extends JPanel implements INotificable {
	...
	private void handleAddSaleItem() {
		...
		try {
			...
			JOptionPane.showMessageDialog(getRootPane(), "Fail to Add Sale Item \n" + e.getMessage(), "Exception",
	...
	private void checkAndGoToPayment() {
		...
		if (paymentDialog == null)
			paymentDialog = new CheckoutPayDialog();
	...
	private void handlePaymentComplete() {
		...
		try {
			if (manager.closeTransaction(transaction)) {
				...
				JOptionPane.showMessageDialog(getRootPane(), "Fail to Save transaction", "Error",
						JOptionPane.ERROR_MESSAGE);
			}
		} catch (TransactionException e) {
			JOptionPane.showMessageDialog(getRootPane(), "Fail to Close transaction\n" + e.getMessage(), "Error",
					JOptionPane.ERROR_MESSAGE);
		}
	...
	private void checkForLowInventoryProduct(ArrayList<SaleItem> saleItems) {
		...
		if (lowInventoryProducts.size() > 0) {
			...
			for (Product product : lowInventoryProducts) {
				productString += product.getProductId() + ", " + product.getProductName() + ", " + " ["
						+ product.getQty() + "]\n";
			}
			JOptionPane.showMessageDialog(getRootPane(), "Inventory Low Alert! \n" + productString, "Inventory",
					JOptionPane.INFORMATION_MESSAGE);
		}
	}
	...
	private void printTransaction(TransactionInterface transaction) {
		...
		for (SaleItem saleitem : transaction.getSaleItems()) {
			data += " \t" + saleitem.getSaleQuantity() + " x " + saleitem.getProduct().getProductName() + "\t\t "
					+ formatDollar(saleitem.getSubTotal()) + "\n";
		}
		data += " ------------------------------------------------------------ ";
	...
	public CheckoutPanel() {
		...
		groupLayout.setVerticalGroup(groupLayout.createParallelGroup(Alignment.LEADING)
				.addGroup(groupLayout.createSequentialGroup().addContainerGap()
						...
						.addComponent(panelLeft, GroupLayout.DEFAULT_SIZE, 297, Short.MAX_VALUE)).addGap(7)));
				...
				.addGroup(gl_panel.createSequentialGroup().addContainerGap()
						.addGroup(gl_panel.createParallelGroup(Alignment.TRAILING)
								...
								.addGroup(gl_panel.createSequentialGroup()
										...
										.addComponent(lblDiscount).addPreferredGap(ComponentPlacement.UNRELATED)
										.addComponent(textFieldDiscountAmount, GroupLayout.PREFERRED_SIZE,
						...
						.addGroup(gl_panel.createSequentialGroup().addComponent(btnDelete)
								...
								.addComponent(textFieldTotalAmount, GroupLayout.PREFERRED_SIZE,
										GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE))
								.addGroup(gl_panel.createSequentialGroup()
										.addComponent(btnPage, GroupLayout.PREFERRED_SIZE, 81,
												GroupLayout.PREFERRED_SIZE)
										.addGap(18)
										.addComponent(btnPay, GroupLayout.PREFERRED_SIZE, 82,
												GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(ComponentPlacement.RELATED, 112, Short.MAX_VALUE)
								.addComponent(lblNewLabel_3).addPreferredGap(ComponentPlacement.UNRELATED)
								.addComponent(textFieldNetAmount, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
										GroupLayout.PREFERRED_SIZE)))
						.addContainerGap()));
		gl_panel.setVerticalGroup(gl_panel.createParallelGroup(Alignment.TRAILING).addGroup(gl_panel
				.createSequentialGroup().addComponent(scrollPane_2, GroupLayout.DEFAULT_SIZE, 167, Short.MAX_VALUE)
				.addPreferredGap(ComponentPlacement.RELATED)
				.addGroup(gl_panel.createParallelGroup(Alignment.BASELINE)
						.addComponent(textFieldTotalAmount, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
								GroupLayout.PREFERRED_SIZE)
						.addComponent(lblTotal).addComponent(btnDelete))
				.addGap(8)
				.addGroup(gl_panel.createParallelGroup(Alignment.BASELINE)
				...
				.addGroup(gl_panel.createParallelGroup(Alignment.BASELINE)
						.addComponent(textFieldNetAmount, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
								GroupLayout.PREFERRED_SIZE)
						.addComponent(lblNewLabel_3).addComponent(btnPage).addComponent(btnPay))
				.addContainerGap()));
